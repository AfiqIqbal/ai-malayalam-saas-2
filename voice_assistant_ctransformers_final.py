...
# Model configuration - Using TinyLlama for better performance
MODEL_REPO = "TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF"
MODEL_FILE = "tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf"  # Quantized model for better performance
    ...
    def __init__(self):
        ...
        try:
            self.model = AutoModelForCausalLM.from_pretrained(
                ...
                model_type="llama",  # TinyLlama is based on LLaMA architecture
                gpu_layers=20,       # Reduced GPU layers for better compatibility
                context_length=1024,  # Reduced context length for better performance
                batch_size=1,
                threads=2,           # Reduced threads to prevent resource exhaustion
                max_new_tokens=256   # Limit response length for faster generation
            )
            logger.info("Language model loaded successfully")
        except Exception as e:
            logger.error(f"Failed to load model: {e}")
            logger.info("Falling back to an even smaller model...")
            # Fallback to a very small model if the main one fails
            self.model = AutoModelForCausalLM.from_pretrained(
...
if __name__ == "__main__":
    ...
    try:
        ...
        import subprocess
        import sys
        subprocess.check_call([sys.executable, "-m", "pip", "install", "ctransformers", "pyaudio", "SpeechRecognition", "gTTS", "pygame"])
