...
class VoiceAssistant:
    ...
    def _get_cached_ollama_response(self, prompt_hash: str, prompt: str) -> str:
        """Internal method to get response from Ollama with caching
        
        Args:
            prompt_hash: MD5 hash of the prompt for caching
            prompt: The actual prompt text to send to Ollama
    ...
    def get_ollama_response(self, prompt: str) -> str:
        """Get response from Ollama's Mistral model with caching
        ...
        response = self._get_cached_ollama_response(prompt_hash, prompt)
        
        # Log cache hit/miss for debugging
        if hasattr(self._get_cached_ollama_response, 'cache_info'):
            cache_info = self._get_cached_ollama_response.cache_info()
            logger.debug(f"Cache info - Hits: {cache_info.hits}, Misses: {cache_info.misses}, Size: {cache_info.currsize}/{cache_info.maxsize}")
